...
class Activated extends Action
{
    ...
    public function execute(): ResultInterface
    {
        ...
        if (!$sessionId) {
            ...
            return $this->resultFactory->create(ResultFactory::TYPE_REDIRECT)->setPath('etechflow_storelocator/license/gate');
        }
        ...
        $block = $page->getLayout()->getBlock('etechflow_storelocator.license.activated');
        if ($block) {
            $block->setData('license_key', $licenseKey)
                  ->setData('plan', $planName)
                  ->setData('error', $error)
                  ->setData('settings_url', $this->getUrl('adminhtml/system_config/edit/section/etechflow_storelocator'))
                  ->setData('management_url', $this->getUrl('etechflow_storelocator/license/gate'));
        }
